...
AUTOENCODER_EPOCHS_CV = 75  # Increased epochs
...
def train_autoencoder_improved(train_data, input_dim, epochs=AUTOENCODER_EPOCHS_CV, lr=LEARNING_RATE, 
                               validation_data=None, patience=15):
    model = ImprovedPhysiologicalAutoencoder(input_dim, latent_dim=16, dropout_rate=0.3)
...
def evaluate_baseline_models(X_train_baseline, X_test_baseline, X_test_stress):
    ...
    try:
        iso = IsolationForest(n_estimators=500, contamination='auto', random_state=RANDOM_SEED)
...
def evaluate_fold(subject, baseline_df, stress_df, feature_columns):
    ...
    n_train = len(X_train_baseline)
    split_idx = int(0.8 * n_train)
    X_train = X_train_baseline[:split_idx]
    X_val = X_train_baseline[split_idx:]

    model, _ = train_autoencoder_improved(
        X_train,
...
def save_error_distribution_and_roc(baseline_scores, stress_scores, filename_prefix='improved'):
    ...
    plt.legend()
    plt.grid(alpha=0.3)
    dist_path = os.path.join(RESULTS_DIR, f'error_distribution_{filename_prefix}.png')
    ...
    plt.grid(alpha=0.3)
    roc_path = os.path.join(RESULTS_DIR, f'roc_curve_{filename_prefix}.png')
...
def save_model_comparison_plot(model_comparison_df, filename_prefix='improved'):
    ...
    out_path = os.path.join(RESULTS_DIR, f'model_auc_comparison_{filename_prefix}.png')
    plt.tight_layout()
...
def train_and_save_final_model(baseline_df, feature_columns):
    ...
    final_model, losses = train_autoencoder_improved(
        X_baseline,
        input_dim=len(feature_columns),
...
def main():
    ...
    try:
        ...
        pooled_auc, dist_path, roc_path = save_error_distribution_and_roc(
            pooled_baseline_scores,
            pooled_stress_scores,
            filename_prefix='improved',
        )
        comp_plot_path = save_model_comparison_plot(model_comparison_df, filename_prefix='improved')
