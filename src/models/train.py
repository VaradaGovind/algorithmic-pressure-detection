...
SCALER_ARTIFACT_PATH = 'models/autoencoder_artifacts.npz'

RANDOM_SEED = 42
BATCH_SIZE = 128
AUTOENCODER_EPOCHS_CV = 30
AUTOENCODER_EPOCHS_FINAL = 50
LEARNING_RATE = 1e-3
...
def train_autoencoder(train_data, input_dim, epochs=AUTOENCODER_EPOCHS_CV, lr=LEARNING_RATE):
...
def evaluate_baseline_models(X_train_baseline, X_test_baseline, X_test_stress):
    ...
    try:
        iso = IsolationForest(n_estimators=300, contamination='auto', random_state=RANDOM_SEED)
...
def save_error_distribution_and_roc(baseline_scores, stress_scores):
    ...
    plt.title('Reconstruction Error Distribution (LOSO)', fontsize=14)
    ...
    plt.title('ROC Curve for Anomaly Detection (LOSO)', fontsize=14)
...
def save_model_comparison_plot(model_comparison_df):
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel('AUC')
    plt.title('Model Comparison under LOSO Cross-Validation')
    plt.grid(axis='y', alpha=0.3)
    out_path = os.path.join(RESULTS_DIR, 'model_auc_comparison.png')
    plt.tight_layout()
...
def train_and_save_final_model(baseline_df, feature_columns):
    ...
    final_model, losses = train_autoencoder(
        X_baseline,
        input_dim=len(feature_columns),
...
def main():
    ...
    try:
        ...
        print(f"Running LOSO cross-validation across {len(subjects)} subjects...")
        ...
        fold_metrics_path = os.path.join(RESULTS_DIR, 'loso_fold_metrics.csv')
        fold_metrics_df.to_csv(fold_metrics_path, index=False)

        window_scores_df = pd.concat(window_score_rows, ignore_index=True)
        window_scores_path = os.path.join(RESULTS_DIR, 'cv_window_scores.csv')
        ...
        summary_lines = [
            ...
            f"LOSO pooled autoencoder AUC: {pooled_auc:.4f}",
            f"Mean autoencoder fold AUC: {fold_metrics_df['autoencoder_auc'].mean():.4f}",
